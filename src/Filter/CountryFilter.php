...
class CountryFilter
{
    public const COUNTRY_DICT = [
        ...
        ['MA', '摩洛哥', 'Morocco', '212'],
        ['MC', '摩纳哥', 'Monaco', '377'],
        ['MD', 'MD', '摩尔多瓦', 'Moldova, Republic of',],
    ...
    /** @var string */
    private static $countryRegex;

    public static function getCountryRegex()
    {
        if (! self::$countryRegex) {
            $countries = [];
            foreach (self::COUNTRY_DICT as $item) {
                $countries[] = $item[1];
            }
            self::$countryRegex = '/^(' . implode('|', $countries) . ')(.*)/';
        }

        return self::$countryRegex;
    }

    public static function filter(string $result): ?array
    {
        if (preg_match(self::getCountryRegex(), $result, $m)) {
            return [$m[1], $m[3]];
        } else {
            return null;
        }
